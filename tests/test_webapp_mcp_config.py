...
class TestWebappMCPConfig(unittest.TestCase):
    ...
    def test_create_session_uses_fallback_name_when_blank(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            ...
            with patch("backend.webapp.ConversationStore", new=conversation_store_factory):
                ...
                self.assertTrue(str(result["session"]["name"]).startswith("会话 "))

    def test_rename_session_endpoint(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            conversation_db = Path(tmp_dir) / "conversations.db"
            ...
            with patch("backend.webapp.ConversationStore", new=conversation_store_factory):
                ...
                create_resp = client.post(
                    "/api/sessions",
                    json={"name": "初始会话", "seedGoal": "", "config": {}},
                )
